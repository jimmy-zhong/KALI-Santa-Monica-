...
*@return	无
*/
static void make_basic_config_setting_json_callback(int8_t *buf, CONFIG_MSG config_msg)
{
  sprintf((char *)buf,"settingsCallback({\"ver\":\"%d.%d\",\
...
{	int8_t *name;
	...
	switch (http_request->METHOD)
	{
		...
		case METHOD_GET:																			/*GET请求方式*/
			...
			else if(strcmp((char *)name,"/dream.htm")==0 || strcmp((char *)name,"/")==0 || (strcmp((char *)name,"/dream.html")==0))
			{
				...
				while(file_len)
				{
					if(file_len>1024)
					{
						...
						send(s, (uint8_t *)DREAM_HTML+send_len, 1024);
						send_len+=1024;
						file_len-=1024;
					}
					else
					{
						send(s, (uint8_t *)DREAM_HTML+send_len, file_len);
						send_len+=file_len;
						file_len-=file_len;
					} 
				}
			...
			else if(strcmp((char *)name,"/w5500.js")==0)
			{
				...
				make_basic_config_setting_json_callback((int8_t *)&tx_buf, ConfigMsg);
		...
		case METHOD_POST:																			/*POST请求*/
			...
			if(strcmp((char *)&req_name, "config.cgi")==0)							  	
			{
				...
				make_cgi_response(5,(int8_t *)ConfigMsg.lip, (int8_t *)&tx_buf);	/*生成响应的文本部分*/        
				sprintf((char *)http_response,"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length:%d\r\n\r\n%s",strlen(tx_buf),tx_buf);
																													/*发送http响应*/
...
{	uint8_t *param;
  ...
  param = get_http_param_value((char *)&(http_request->URI), "ip");		/*获取修改后的IP地址*/
  if(param)
  {
    inet_addr_((uint8_t *)param, ConfigMsg.lip);	
  }
  param = get_http_param_value((char *)&(http_request->URI), "gw");		/*获取修改后的网关*/
  if(param)
  {
    inet_addr_((uint8_t *)param, ConfigMsg.gw);	
  }
  param = get_http_param_value((char *)&(http_request->URI), "sub");	/*获取修改后的子网掩码*/
  if(param)
  {
...
*@return	无
*/
void make_cgi_response(uint16_t delay, int8_t *url, int8_t *cgi_response_buf)
{
  sprintf((char *)cgi_response_buf,"<html><head><title>iWeb - Configuration</title><script language=javascript>j=%d;function func(){document.getElementById('delay').innerText=' '+j + ' ';j--;setTimeout('func()',1000);if(j==0)location.href='http://%d.%d.%d.%d/';}</script></head><body onload='func()'>please wait for a while, the module will boot in<span style='color:red;' id='delay'></span> seconds.</body></html>",delay,url[0],url[1],url[2],url[3]);
  return;
}
