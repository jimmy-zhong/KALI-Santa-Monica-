...
void IINCHIP_WRITE( uint32_t addrbsb,  uint8_t data)			// MCU将通过SPI获取的数据写入自身的接收缓存
{
...
uint8_t IINCHIP_READ(uint32_t addrbsb)										// MCU将通过SPI获取的数据读取并返回数据值
{	uint8_t data = 0;																				// 定义读取的数据值
...
uint16_t wiz_write_buf(uint32_t addrbsb, uint8_t *buf, uint16_t len)	// W5500将通过SPI获取的数据写入相关寄存器，并返回写入的数据长度
...
uint16_t wiz_read_buf(uint32_t addrbsb, uint8_t *buf, uint16_t len)	// W5500将通过SPI获取的数据读出，并返回读取的数据长度
{	uint16_t idx = 0;																			// idx定义为正在读取的第几个数
