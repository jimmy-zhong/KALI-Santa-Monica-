...
//	sprintf((char*)host_name,"%3s-%02X%02X%02X",DEVICE_TYPE,DHCP_CHADDR[3],DHCP_CHADDR[4],DHCP_CHADDR[5]); 
...
{	uint8_t type;
	...
	switch (dhcp_state)
	{
		...
		case STATE_DHCP_DISCOVER:								// STATE_DHCP_DISCOVER=1
			ip_set=0;
			if (type == DHCP_OFFER)								// DHCP_OFFER=2
			{
